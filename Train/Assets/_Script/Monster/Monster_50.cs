...
public class Monster_50 : Monster
{
    [SerializeField]
    Vector3 movement;
    int xPos;
    ...
    IEnumerator BulletFire()
    {
        ...
        if (monster_gametype != Monster_GameType.GameEnding && monster_gametype != Monster_GameType.Die)
        {
            for (int i = 0; i < 5; i++)
            {
                ...
                bullet.GetComponent<Monster_Bullet_Down>().Get_MonsterBullet_Information(Bullet_Atk - (int)Item_Monster_Atk, Bullet_Slow, Bullet_Speed, 0);
                Vector2 Pos = new Vector2(Fire_Zone.position.x + (1f * i * xPos), Fire_Zone.position.y);
                Instantiate(bullet, Pos, transform.rotation, monster_Bullet_List);
    ...
    public void changeAtkFlag(bool flag)
    {
        atkFlag = flag;
    }
}
