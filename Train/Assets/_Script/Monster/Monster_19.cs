...
public class Monster_19 : Monster
{
    [SerializeField]
    Vector3 movement;
    int xPos;
    ...
    IEnumerator SpawnMonster()
    {
        ...
        while (elapsedTime < duration)
        {
            ...
            float yPos = Mathf.Sin(Mathf.PI * t) * height + Mathf.Lerp(Spawn_Init_Pos.y, MonsterDirector_Pos.y + 1f, t); ;
            transform.localPosition = new Vector2(xPos, yPos);
            //Debug.Log(yPos);

            yield return null;
        }
        MonsterDirector_Pos = new Vector2(MonsterDirector_Pos.x, MonsterDirector.MinPos_Ground.y + 1f);
    ...
    public void BulletFire()
    {
        if (!DieFlag)
        {
            ...
            GameObject bullet2 = Instantiate(BulletObject, Fire_Zone.position, transform.rotation, monster_Bullet_List);
            bullet2.GetComponent<MonsterBullet>().Get_MonsterBullet_Information(Bullet_Atk - (int)Item_Monster_Atk, Bullet_Slow, Bullet_Speed, xPos);
        }
