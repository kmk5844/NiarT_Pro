...
public class Monster_57 : Monster
{
    [SerializeField]
    Vector3 movement;
    int xPos;
    ...
    IEnumerator _BulletFire()
    {
        ...
        if (monster_gametype != Monster_GameType.GameEnding)
        {
            for (int i = 0; i < 8; i++)
            {
                ...
                bullet.GetComponent<MonsterBullet>().Get_MonsterBullet_Information(Bullet_Atk - (int)Item_Monster_Atk, Bullet_Slow, Bullet_Speed);
                float xPos = Random.Range(-4f, 4f);
                Vector2 Rand = new Vector2(Fire_Zone.position.x + xPos, Fire_Zone.position.y);
                Instantiate(bullet, Rand, transform.rotation, monster_Bullet_List);
