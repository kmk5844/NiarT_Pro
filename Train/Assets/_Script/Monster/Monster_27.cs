...
public class Monster_27 : Monster
{
    Vector3 movement;
    float xPos;
    ...
    void BulletFire()
    {
        ...
        if (Time.time >= lastTime + (Bullet_Delay + Item_Monster_AtkDelay) && monster_gametype != Monster_GameType.Die)
        {
            ...
            bullet.GetComponent<MonsterBullet>().Get_MonsterBullet_Information(Bullet_Atk - (int)Item_Monster_Atk, 0, 0, 0);
            bullet.transform.localPosition = new Vector2(transform.localPosition.x, MonsterDirector.MinPos_Ground.y + 1);
            bullet.GetComponent<MonsterBullet>().SetDestory(5f);
            lastTime = Time.time;
        }
    }
